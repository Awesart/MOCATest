...
interface UserApi{
    ...
    suspend fun getLocalUsers(userSession: UserSession?): Result<LocalUserListDto, DataError>
}
...
class UserSessionNetworkApi (
    ...
    override suspend fun getLocalUsers(
        userSession: UserSession?
    ): Result<LocalUserListDto, DataError> {
        val connectionString = "user/getLocal"
        ...
        return try{
            ...
            if (response.status.value in 200..299) {
                ...
                val localUserList: LocalUserListDto = response.body()

                Result.Success(localUserList)
            }
            else{
                when(response.status.value){
                    401 -> Result.Error(DataError.Network.NOT_AUTHORIZED)
